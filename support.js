...
(() => {
  ...
  // src/parse.ts
  /* STRIPS THE <template data-dc-body> WRAPPER A .dc.html FILE MAY PUT ROUND ITS BODY.
   *
   ...
   * gives every valueless attribute a value -- `<template data-dc-body="">`. A pattern tight enough to require
   * one silently fails on the other, and the failure mode is a blank page with every module loaded and no error
   * anywhere, which is a genuinely hard thing to debug. Hence [^>]* on both sides of the attribute name.
   *
   * data-dc-body IS REQUIRED so this can never swallow a template a component authored on purpose. Files without
   * the wrapper are returned untouched, which is the path the other seven .dc.html files here still take.
   */
  function unwrapDcBody(html) {
    return html.replace(/<template\b[^>]*\bdata-dc-body\b[^>]*>([\s\S]*)<\/template\s*>/i, "$1");
  }
  function parseDcDocument(doc) {
    ...
    return {
      template: unwrapDcBody(dc.innerHTML),
  ...
  function parseDcText(src) {
    ...
    const template = unwrapDcBody(src.slice(openMatch.index + openMatch[0].length, close));
