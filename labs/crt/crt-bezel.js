/* crt/crt-bezel.js — the frame's colors.
 *
 * Pure. The bezel is PLASTIC: it takes a little of the screen's phosphor color and a little of the room light's color,
 * but only a little — plastic scatters rather than mirrors. The base color comes straight from the picker and the tint is
 * mixed ON TOP of it rather than replacing it, so the molding still reads as one material whatever color is chosen:
...
/* s = the settings, phosphor = the active phosphor. Returns the stroke colors and widths for the bezel's four SVG paths.
 *
 * The tint source depends on what is actually emitting: the phosphor while the tube is lit, the fixture's color
...
export function bezelCols(s, phosphor) {
  ...
  /* FRAME LIGHT scales the lift the lamp gives the molding's lit edge. 50% is neutral -- the 0.28 bezelCols always applied --
   ...
   * that to 0.126-0.25: a few RGB points per step on a dark molding. The 1.6 exponent leaves the midpoint exactly where it
   ...
   * reads as a molding with depth rather than a flat lit ring.
   */
  const glow = Math.max(0, Math.min(2, (s.fglow == null ? 50 : s.fglow) / 50));
  const lift = Math.pow(glow, 1.6);
  /* THE LIT EDGE IS LIT BY SOMETHING, and until now it was not. `hi` was mixed from the molding's own color and pure
   ...
   * the tube's own face, which throws enough light on a surrounding molding to keep its edge readable in a dark room. At
   * a lamp on full the strength reaches 1.0, so the frame under a working lamp looks exactly as it always did; with the
   * lamp off it drops to the tube's contribution alone, and with everything off the molding goes nearly flat -- dead
   * plastic, which is what unlit plastic looks like.
   *
   * THE HIGHLIGHT ALSO TAKES THE LAMP'S COLOR rather than staying white, for the same reason the tint does: a 3200K bulb
  ...
  return {
    ...
    hi: rgbStr(mix(baseRgb, litCol, litAt(1))),   // the sheen along the lit edge of the molding
