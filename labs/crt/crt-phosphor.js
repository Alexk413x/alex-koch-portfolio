/* crt/crt-phosphor.js — colour. The phosphor palette, the blackbody table, and the three mixers everything else uses.
 *
 * Pure: no DOM, no component state. This is the bottom of the stack — crt-fixture imports it, nothing here imports
 * anything. Every colour in the instrument comes from one of these four functions, which is what keeps the tube body,
...
export const PH = {
   ...
   * rather than a white phosphor, and the blue reached everything downstream: the glow colour feeds the bloom, the phosphor
   ...
   * to keep the contrast would be to make the body text grey, and a white phosphor whose text is grey is not white. An
...
/* THE COLOUR PARSER. Hex in, [r,g,b] out -- and rgb()/rgba() in too, which it did not use to accept.
 *
 ...
 *
 * Parsing here rather than normalising resolvePhosphor's output, because fg cannot change notation: crt-bezel feeds it to
...
export function hexRgb(h) {
  ...
  if (m) {
    ...
    // Alpha is dropped on purpose: every consumer wants a colour to mix, not a compositing instruction.
 ...
 * entries so the colour varies smoothly at any slider resolution. */
...
// The active phosphor, named or custom. A custom colour derives its highlight rather than taking a second picker: one
// colour in, and the text and its caret cannot be set to an unreadable pair.
